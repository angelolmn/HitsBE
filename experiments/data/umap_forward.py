...
for idx in range(N):
    df = pd.read_csv(f"experiments/data/model_forward_shape/cls_{idx}.csv")
    ...
    plt.title(f"Capa {idx} del modelo", fontsize=16)

    plt.tick_params(axis='both', labelsize=16)
    plt.legend(markerscale=2, bbox_to_anchor=(1.04, 1), loc="upper left", borderaxespad=0)
    #plt.tight_layout()

    save_root = f"../memoriatfg/img/experimentos/model_forward_shape/"
    os.makedirs(save_root, exist_ok=True)
    plt.savefig(save_root + "cls_" + str(idx) + ".png", dpi=300)
